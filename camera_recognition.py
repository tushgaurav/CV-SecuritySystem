...
def dropbox_sms(filename, msg):
  ...
  # file name
    ...
    '''
    client.api.account.messages.create(to="PHONE NUMBER HERE",
                                       from_="YOUR NUMBER",
                                       body=msg,
                                       media_url=media)
...
def unknown_msg(msgcount, frame, visitor_name):
    ...
    msgcount += 1
    msg = "Admin Alert: \n" + \
        str(visitor_name) + " person entered the facility, at " + \
    ...
    def get_frame(self):
        ...
        if ret:
            ...
            for (x, y, w, h) in faces:
                ...
                conn = c.cursor()
                cur = conn.execute(
                    "SELECT visitor_name FROM log WHERE id = (SELECT MAX(id) FROM log)")
                name_q = cur.fetchone()
                if name_q != None:
                    name_q = str(name_q[0])
                else:
                ...
                if (conf < 70):
                    ...
                    cv2.putText(
                        frame, profile[1], (x, y + h), font, 1.0, (0, 255, 0), 2)
                    cv2.putText(frame, str(
                        profile[2]), (x, y + h + 30), font, 1.0, (0, 255, 0), 2)
                    cv2.putText(
                        frame, profile[3], (x, y + h + 60), font, 1.0, (0, 255, 0), 2)
                    ...
                    if prev_profile != profile and name_q != profile[1]:
                        ...
                        conn = c.cursor()
                        conn.execute(
                            "INSERT INTO log (visitor_name, time) VALUES (?,?)", (profile[1], str(cur_time)))
                        c.commit()
                        conn.close()

                        thread = threading.Thread(
                            target=visitor_msg, args=(msgcount, frame, profile[1]))
                    ...
                    if name_q != 'Unknown':
                        ...
                        conn = c.cursor()
                        conn.execute(
                            "INSERT INTO log (visitor_name, time) VALUES (?,?)", ('Unknown', str(cur_time)))
                        c.commit()
                        conn.close()

                        thread = threading.Thread(
                            target=unknown_msg, args=(msgcount, frame, 'Unknown'))
                        thread.daemon = True  # Daemonize thread
                        thread.start()  # Start the execution
